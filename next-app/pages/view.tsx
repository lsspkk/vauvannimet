...
import { useStateValue, setUsername, loadHearts, setHearts, setRound } from 'components/state/state'
...
function maxRound(h: HeartInterface[]) {
  const rounds = h.map(h => !h.rounds || h.rounds.length === 0 ? 0 : Math.max(...h.rounds.map(r => r.round)))
    ...
    <Layout {...{ user, loading }}>
      <div className='flex justify-between items-center w-full flex-col'>
        ...
        <div className='mt-2'>
          ...
          <div className='pr-[5rem]'>{round === 0 ? 'Alkukierros' : `Jatkokierros ${round}`}</div>
          <RoundMenu />
          </div>
                    {round === 0 &&
          Array.from(results.keys()).map((key) => (
            <div key={key}>
              ...
              <div className='flex flex-wrap'>
                {results.get(key)?.sort(({score:a, name: na}, {score:b, name: nb})=> {
                  if( b-a ) return b-a
                  if( na < nb ) return -1
                  if( na > nb ) return 1
                  return 0
...
function RoundMenu() {
  ...
  return <div >
    ...
    {rounds.filter(r => r!==round).map(r => 
      (<Button className={`rounded-full w-full ${r===round ? 'bg-red-200': ''}`} 
      key={`chooseRound.${r}`} disabled={r === round} onClick={() => dispatch(setRound(r))}>
      {r}</Button>)) }
      </div>}
      {round > 0 && <Button className='rounded-full w-full' onClick={() => dispatch(setRound(0))}>X</Button> }
  </div>
}
...
function ChooseWithRound({user}: {user: Account}) {
  ...
  const uniqueHearts = Array.from(uniqueNames).map(name => ({name, nameHearts: hearts.filter(h => h.name === name)})).sort((a,b)=> {
    if( a.name > b.name) return 1
    if( a.name < b.name) return -1
    return 0
  })
  return <table className='w-[100%] mt-8' >
    <tr>
      <td className='w-40'>
    <Title>
      ...</Title>
    </td>
    {user?.usernames.map((name) => (<td key={`tuk.${round}.${name}`} className='align-center text-center'>{name}</td>))}
    ...
    </tr>
{
  uniqueHearts.map((uh,i) => 
    (<tr key={`chooseround.${round}.${uh.name}`} className={i%2 === 0 ? 'bg-gray-100': ''}>
...
function ChooseNameRound({enabled,name,hearts: myHearts, chooser }: {chooser: string, enabled: boolean, name: string, hearts: HeartInterface[]}) {
  const [{ round, hearts, username }, dispatch] = useStateValue()
  const scoreRange = Array.from(Array(5), (x, i) => i + 1)

  if( myHearts.length < 1 ) { return <div/>}
  ...
  function handleScoreClicked(score: number) {
    ...
    if( oldScore && oldRounds ) {
      rounds = oldRounds.map(r => (r.round === round && r.scores 
        ? {...r, round, scores: r.scores.map(s => s.username !== username ? {...s} : newScore)} 
        : {...r}))
    }
    else if( oldRound && oldRounds ) {
      rounds = oldRounds.map(r => r.round === round && r.scores 
        ? {...r, scores: r.scores ? [...r.scores, newScore] 
          : [newScore]} : {...r})
    }
    else if( oldRounds ) {
    ...
    const changedHearts: HeartInterface[] = hearts.map(h => (h.name === name ? {...h, rounds}: {...h}))
    dispatch(setHearts(changedHearts))
  }

  return (
    <div className='flex flex-col sm:pl-4 pr-2 py-4'>
    <div className={`flex ${!enabled ? 'flex-col md:flex-row' : ''} items-center align-items-center justify-center`}>
    {scoreRange.map((score) => (
      <HeartIcon
        key={`scoreRange.${username}.${name}.${score}`}
        className={`${enabled ? 'h-5 w-5 mx-1 `md:h-6 md:w-6': 'h-2 w-2 mx-[2px] md:h-5 md:w-5 opacity-50'}`}
